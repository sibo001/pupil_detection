...
typedef struct
{
  ...
  std::vector<int> Features;  /* Pointer to the features vector */
  std::vector<float> Weights;       /* Pointer to the weights of the features */
} signature_t;
...
//THE MAIN FUNCTION//
float emd(signature_t *Signature1, signature_t *Signature2, float (*Dist)(int, int), flow_t *Flow, int *FlowSize){
    int itr;
    double totalCost;
    float w;
    node2_t *XP;
    ...
    if (_n1 > 1 && _n2 > 1){  /* IF _n1 = 1 OR _n2 = 1 THEN WE ARE DONE */
        ...
        if (itr == MAX_ITERATIONS) fprintf(stderr, "emd: Maximum number of iterations has been reached (%d)\n",MAX_ITERATIONS);
    }

    /* COMPUTE THE TOTAL FLOW */
    totalCost = 0;
    if (Flow != NULL) FlowP = Flow;
    for(XP=_X; XP < _EndX; XP++){
        if (XP == _EnterX) continue;
        if (XP->i == Signature1->n || XP->j == Signature2->n) continue;
        if (XP->val == 0) continue;
    ...
    if (Flow != NULL)*FlowSize = FlowP-Flow;
...
//init
static float init(signature_t *Signature1, signature_t *Signature2, float (*Dist)(int, int)){
  ...
  _maxC = 0;
  P1 = Signature1->Features;
  P2 = Signature2->Features;
  for(int i=0; i < _n1; i++){
    for(int j=0; j < _n2; j++){
        _C[i][j] = Dist(P1[i], P2[j]);
        if (_C[i][j] > _maxC) _maxC = _C[i][j];
    }
  }
